<?
class CDataToolsIblock
{  
  
    function Add($arFields)
    {
        global $DB;
        $arInsert = $DB->PrepareInsert("b_datatools1c_iblock", $arFields);
        $strSql =
            "INSERT INTO b_datatools1c_iblock(".$arInsert[0].") ".
            "VALUES(".$arInsert[1].")";
    ...
    function Update($ID ,$arFields)
    {
        global $DB;
        $ID = IntVal($ID);  
        $strUpdate = $DB->PrepareUpdate("b_datatools1c_iblock", $arFields);  
        $strSql = "UPDATE b_datatools1c_iblock SET ".$strUpdate." WHERE ID=".$ID;
    ...
    function GetIdByForPropertyId($ORDER_ID)
    {
        ...
        $strSql = "SELECT P.ID FROM b_datatools1c_iblock P WHERE P.FOR_PROPERTY_ID='".$DB->ForSql($ORDER_ID)."'";
    ...
    function GetById($ID)
    {
        global $DB;    
        $ORDER_ID = IntVal($ID);
        $strSql = "SELECT P.* FROM b_datatools1c_iblock P WHERE P.ID='".$DB->ForSql($ID)."'";
        $dbRes = $DB->Query($strSql, true);
        $arRes = $dbRes->Fetch();
        return $arRes;        
    }      
    ...
    function GetByIblockId($ID)
    {
        global $DB;    
        $ORDER_ID = IntVal($ID);
        $strSql = "SELECT P.* FROM b_datatools1c_iblock P WHERE P.IBLOCK_ID='".$DB->ForSql($ID)."'";
        $dbRes = $DB->Query($strSql, true);
        $arRes = $dbRes->Fetch();
        return $arRes;        
    }      
    ...
    function Delete($ID)
    {
        global $DB;
        $ID = IntVal($ID);
        $strSql = "DELETE FROM b_datatools1c_iblock WHERE ID='".$DB->ForSql($ID)."'";
    ...
    function DeleteByIblockId($ID)
    {
        global $DB;
        $ID = IntVal($ID);
        $strSql = "DELETE FROM b_datatools1c_iblock WHERE IBLOCK_ID='".$DB->ForSql($ID)."'";
        $DB->Query($strSql, true);
        return true; 
    }
    ...
    function DeleteByForPropertyId($ID)
    {
        global $DB;
        $ID = IntVal($ID);
        $strSql = "DELETE FROM b_datatools1c_iblock WHERE FOR_PROPERTY_ID='".$DB->ForSql($ID)."'";
        $DB->Query($strSql, true);
        return true; 
    }    

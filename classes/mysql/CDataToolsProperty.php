<?
class CDataToolsProperty
{  
    function Add($arFields)
    {
        global $DB;
        $arInsert = $DB->PrepareInsert("b_datatools1c_property", $arFields);
        $strSql =
            "INSERT INTO b_datatools1c_property(".$arInsert[0].") ".
            "VALUES(".$arInsert[1].")";
    ...
    function Update($ID ,$arFields)
    {
        global $DB;
        $ID = IntVal($ID);  
        $strUpdate = $DB->PrepareUpdate("b_datatools1c_property", $arFields);  
        $strSql = "UPDATE b_datatools1c_property SET ".$strUpdate." WHERE ID=".$ID;
    ...
    function UpdateByPropertyId($ID ,$arFields)
    {
        ...
        $ID = IntVal($ID);  
        $strUpdate = $DB->PrepareUpdate("b_datatools1c_property", $arFields);  
        $strSql = "UPDATE b_datatools1c_property SET ".$strUpdate." WHERE PROPERTY_ID=".$ID;
        $res = $DB->Query($strSql, true, $err_mess.__LINE__);
        if($res == false) {
            return false;    
        }
    ...
    function GetIdByForPropertyId($ORDER_ID)
    {
        ...
        $strSql = "SELECT P.ID FROM b_datatools1c_property P WHERE P.FOR_PROPERTY_ID='".$DB->ForSql($ORDER_ID)."'";
    ...
    function GetByID($ID)
    {
        global $DB;    
        $ORDER_ID = IntVal($ID);
        $strSql = "SELECT P.* FROM b_datatools1c_property P WHERE P.ID='".$DB->ForSql($ID)."'";
        $dbRes = $DB->Query($strSql, true);
        $arRes = $dbRes->Fetch();
        return $arRes;        
    }      
    ...
    function GetByPropertyId($ID)
    {
        global $DB;    
        $ORDER_ID = IntVal($ID);
        $strSql = "SELECT P.* FROM b_datatools1c_property P WHERE P.PROPERTY_ID='".$DB->ForSql($ID)."'";
        $dbRes = $DB->Query($strSql, true);
        $arRes = $dbRes->Fetch();
        return $arRes;        
    }      
    ...
    function Delete($ID)
    {
        global $DB;
        $ID = IntVal($ID);
        $strSql = "DELETE FROM b_datatools1c_property WHERE ID='".$DB->ForSql($ID)."'";
    ...
    function DeleteByPropertyId($ID)
    {
        global $DB;
        $ID = IntVal($ID);
        $strSql = "DELETE FROM b_datatools1c_property WHERE PROPERTY_ID='".$DB->ForSql($ID)."'";
        $DB->Query($strSql, true);
        return true; 
    }

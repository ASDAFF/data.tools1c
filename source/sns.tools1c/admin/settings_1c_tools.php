...
if(isset($_REQUEST['update'])){
  CSnsToolsEvent::update_table_profile();
}
...
if(($_GET["mode"] == "import") || ($_GET["mode"] == "checkauth")) {
    ...
    if ($_GET["mode"] == "checkauth" && $USER->IsAuthorized()) {
        ...
        if ($submit) {
            //«десь работаем с содержимым переданного файла.
            ...
            if (!is_uploaded_file($tmp_name)) {
                ...
                //“еперь нормальный файл можно сохранить на диске
...
function OptionGetValue($key) {
    $result = COption::GetOptionString("sns.tools1c",$key);
    if($_REQUEST[$key]){
        $result = $_REQUEST[$key];
    }
    return $result;
...
//фотографии в торговые предложени€
...
$showRightsTab - определ€ет надо ли показывать вкладку с настройками прав доступа к модулю ( true / false )
*/

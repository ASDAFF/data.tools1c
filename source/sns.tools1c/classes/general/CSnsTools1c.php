...
use Sns\Tools1c\EventsTable;
use Sns\Tools1c\ChecksTable;
use Sns\Tools1c\ProfileTable;
use Sns\Tools1c\LogHandler;
use Sns\Tools1c\OrderTable;
...
class CSnsToolsEvent
{
    ...
    function GetPropertyNameIblock_NoCache($IBLOCK_ID=false, $FOR_PROPERTY_ID=false, $PROPERTY_TYPE=false) {
        $vars = array();
        $properties = CSnsToolsProperty::GetList(array(), array('PROPERTY_IBLOCK'=>$IBLOCK_ID, 'FOR_PROPERTY_ID'=>$FOR_PROPERTY_ID, 'PROPERTY_TYPE'=>$PROPERTY_TYPE));
    ...
    function GetIblockDirectoryByProperty_NoCache($FOR_PROPERTY_ID=false, $TYPE=false) {
        ...
        $Iblockres = CSnsToolsIblock::GetList(array(), $fillterIblock, array(''));
    ...
    function GetArticleElementByOffers_NoCache($IBLOCK_OFFERS_ID=false) {
        ...
        $PROP_ATICLES = COption::GetOptionString('sns.tools1c' , "CATALOG_ARTICLE");
        $IB_INFO = CCatalogSKU::GetInfoByOfferIBlock($IBLOCK_OFFERS_ID);
    ...
    function GetMorePhotoElementByOffers_NoCache($IBLOCK_OFFERS_ID=false) {
        ...
        $PROP_MORE_PHOTO = COption::GetOptionString('sns.tools1c' , "CATALOG_MORE_PHOTO");

        $Descript_File_Iblock = CSnsToolsEvent::GetDescriptionFileIblock();
    ...
    function GetAllDiscontTools_NoCache($lid='') {
        ...
        $DISCOUNT_CONDITION_BASKET = COption::GetOptionString('sns.tools1c' , "DISCOUNT_CONDITION_BASKET");
        if($DISCOUNT_CONDITION_BASKET == 'Y'){
            $Discontkres = CSnsToolsDiscount::GetList(array(), array(), array());
    ...
    function GetDiscontProduct_NoCache($id) {
        ...
        $resIdDiscont1cTools = CSnsToolsDiscount::GetList(array(), array(), array());
        while($ar_IdDiscont1cTools = $resIdDiscont1cTools->Fetch()) {
            $IdDiscont1cTools[] = $ar_IdDiscont1cTools['DISCONT_ID'];
        }


        $type = COption::GetOptionString('sns.tools1c' , "DISCOUNT_CONDITION_BASKET");
    ...
    function GetUniqueProp($code=false) {
        ...
        if($_GET['type'] == 'catalog' && $_GET['mode'] == 'import') {
            ...
            $props = CSnsToolsEvent::GetAllPropertyIblock();
    ...
    function OffersAttributesSeparate(&$arFields=array() , $type=false , $optionProper=false) {
        ...
        if($_GET['type'] == 'catalog' && $_GET['mode'] == 'import' && strstr($_GET['filename'], $type)) {
            ...
            $PROPS_GET = COption::GetOptionString('sns.tools1c' , $optionProper);
            ...
            $elemProper = CSnsToolsEvent::GetPropertyElementIblock($arFields['IBLOCK_ID'], $arFields['ID']);
            if(empty($elemProper)){
                $elemProper = array();
            }
            if(count($elemProper)>0){
                foreach($elemProper as $ob) {
                    if($ob['CODE'] == $PROPS_GET) {
                        ...
                        $properties = CSnsToolsEvent::GetPropertyNameIblock($arFields['IBLOCK_ID'], $ob["ID"], 'S');
                        ...
                        if(empty($propSave_S)) {
                            ...
                            $propSave_S = CSnsToolsEvent::GetCleanCode($propSave_S);
    ...
    function OffersAttributesProperList(&$arFields=array() , $type=false , $optionProper=false) {
        ...
        if($_GET['type'] == 'catalog' && $_GET['mode'] == 'import' && strstr($_GET['filename'], $type)) {
            ...
            $OFFERS_ATTRIBUTES = COption::GetOptionString('sns.tools1c' , "OFFERS_ATTRIBUTES" , "CML2_ATTRIBUTES");
    ...
    function GetMultipleProperElement($SETTINGS=array()) {
        ...
        if($SETTINGS['STOP_CANCEL'] != 'Y'){
            ...
            $CHEXBOX_MULTIPROPER_OFFERS = COption::GetOptionString('sns.tools1c' , "CHEXBOX_MULTIPROPER_OFFERS");
        ...
        //START
        $STRING_MULTIPROPER_ID_NO = COption::GetOptionString('sns.tools1c' , "STRING_MULTIPROPER_ID_NO");
        ...
        $elemProper = CSnsToolsEvent::GetPropertyElementIblock($SETTINGS['IBLOCK_ID'], $SETTINGS['ID']);
        if(empty($elemProper)){
            $elemProper = array();
        }
        if(count($elemProper)>0){
    ...
    function AddPropDirectory($SETTING=array(), $arFields=array()) {
        ...
        $allPropsArr = CSnsToolsEvent::GetAllPropertyIblock();


        $resProperMultiple = CSnsToolsEvent::GetMultipleProperElement(array(
            'ID' => $SETTING['ID'],
        ...
        $properties = CSnsToolsEvent::GetPropertyNameIblock($SETTING['IBLOCK_ID'], $SETTING['FOR_PROPERTY_ID'], 'E');
        ...
        if(count($properties) == 1 && !in_array($SETTING["FOR_PROPERTY_CODE"],array('CML2_ATTRIBUTES','CML2_TRAITS')) ){
            ...
            if($properties[0]['NAME_1C'] != $SETTING['NAME_SAVE']){
                CSnsToolsProperty::Update($properties[0]['ID'],array('NAME_1C'=>$SETTING['NAME_SAVE']));
                global $CACHE_MANAGER;
                $CACHE_MANAGER->ClearByTag(self::MODULE_ID.'_GetPropertyNameIblock');
            }
        ...
        if(empty($propSave_E)){
            ...
            $propSave_E = CSnsToolsEvent::GetCleanCode($propSave_E);
        ...
        // START
        $Iblock = CSnsToolsEvent::GetIblockDirectoryByProperty($propSave_E);
        if(empty($Iblock)) {

            $siteId = CSnsToolsEvent::GetIblockSiteById($SETTING['IBLOCK_ID']);
        ...
        $CHEXBOX_GOODS_PROPERTIES_CODE = COption::GetOptionString('sns.tools1c', "CHEXBOX_GOODS_PROPERTIES_CODE");
        if($Iblock && $propSave_E) {
            if($CHEXBOX_GOODS_PROPERTIES_CODE == 'Y'){
                $arrElementDirectory = CSnsToolsEvent::GetDirElement($Iblock, $SETTING['XML_ID']);
            }else{
                $arrElementDirectory = CSnsToolsEvent::GetDirectoryElement($Iblock);
            }
    ...
    function GetArrayPropsValueNoAdd($iblockId=false)
    {
        ...
        if($iblockId)
        {
            ...
            {
                $str = COption::GetOptionString('sns.tools1c' , "OFFERS_PROPERTIES_HIGHLOAD_NOT_VALUE_PROPS");
            }
            else   //если инфоблок не является инфоблоком торогых
            {
                $str = COption::GetOptionString('sns.tools1c' , "GOODS_PROPERTIES_HIGHLOAD_MUST_VALUE_NOT");
            }
            $str = trim($str);
            if(strlen($str)>0)
            {
    ...
    function AddPropDirectoryHighLoad($SETTING=array(), $arFields=array()) {
        ...
        //получим свойство куда записать
        //START
        $properties = CSnsToolsEvent::GetPropertyNameIblock($SETTING['IBLOCK_ID'], $SETTING['FOR_PROPERTY_ID'], 'S:directory');
        ...
        if(empty($propSave_S_DIRECTORY)) {
            ...
            $propSave_S_DIRECTORY = CSnsToolsEvent::GetCleanCode($propSave_S_DIRECTORY);
        ...
        // получим инфоблок для записи
        // START
        $Iblock = CSnsToolsEvent::GetIblockDirectoryByProperty($propSave_S_DIRECTORY, 'HIGHLOAD');

        //Получим все инфоблоки,  удалим связь если highLoad удалили
        $IblockHL = CSnsToolsEvent::GetHighLoadIblock();
        if($Iblock && empty($IblockHL[$Iblock])) {

            CSnsToolsIblock::DeleteByIblockId($Iblock);
            global $CACHE_MANAGER;
        ...
        if(empty($Iblock)) {
            ...
            $highload_name = CSnsToolsEvent::GetCleanCode($highload_name);
            ...
            $highload_table = CSnsToolsEvent::GetCleanCode($highload_table);
            $highload_table = substr($highload_table, 0, 42);
            $highload_table = 'b_snstools1c_hl_'.$highload_table.'_'.$propSave_S_DIRECTORY;
            ...
            if($Iblock){
                ...
                CSnsToolsProperty::UpdateByPropertyId($propSave_S_DIRECTORY,
                    array(
        ...
        if($Iblock && $propSave_S_DIRECTORY) {
            ...
            $arrElementDirectory = CSnsToolsEvent::GetDirectoryHighLoadElement($Iblock);
            $IblockHL = CSnsToolsEvent::GetHighLoadIblock();
            ...
            if(count($SETTING['VALUE_SAVE'])>0){
                ...
                $PropsNotAdd =  CSnsToolsEvent::GetArrayPropsValueNoAdd($SETTING['IBLOCK_ID']);

                foreach($SETTING['VALUE_SAVE'] as $value_save) {
                    if($value_save && !in_array($value_save, $PropsNotAdd)){

                        $element_xml_id = Cutil::translit($value_save,'ru', array('change_case' => 'U'));
                        $element_xml_id = CSnsToolsEvent::GetCleanCode($element_xml_id);
    ...
    function AddPropList($SETTING=array(), $arFields=array()) {
        ...
        //получим свойство куда записать
        //START
        $properties = CSnsToolsEvent::GetPropertyNameIblock($SETTING['IBLOCK_ID'], $SETTING['FOR_PROPERTY_ID'], 'L');
        ...
        if(empty($propSave_L)) {
            ...
            $propSave_L = CSnsToolsEvent::GetCleanCode($propSave_L);
        ...
        $arrPropertyValueList = CSnsToolsEvent::GetPropertyValueList($propSave_L);
        //  printr($SETTING);
    ...
    function OffersDirectoryLink(&$arFields=array()) {
        ...
        if($_GET['type'] == 'catalog' && $_GET['mode'] == 'import' && strstr($_GET['filename'], 'offers')) {
            ...
            if(count($elemProper)>0){
                foreach($elemProper as $ob) {
                    if($ob['VALUE'] && $ob['CODE'] == $OFFERS_ATTRIBUTES) {
                        $OFFERS_PROPERTIES_IBLOCK = COption::GetOptionString('sns.tools1c' , "OFFERS_PROPERTIES_IBLOCK");

                        if(in_array($ob['PROPERTY_TYPE'], array('S','L','N'))) {

                            CSnsToolsEvent::AddPropDirectory(
                                array(
                                    'ID' => $arFields['ID'],
                                    'IBLOCK_ID' => $arFields['IBLOCK_ID'],
    ...
    function OffersDirectoryHighLoadLink(&$arFields=array()) {
        ...
        if($_GET['type'] == 'catalog' && $_GET['mode'] == 'import' && strstr($_GET['filename'], 'offers')) {
            ...
            $OFFERS_PROPERTIES_HIGHLOAD_NOT_CREATE = COption::GetOptionString('sns.tools1c' , "OFFERS_PROPERTIES_HIGHLOAD_NOT_CREATE");
            ...
            if(count($elemProper)>0){
                ...
                $CHEXBOX_OFFERS_PROPERTIES_HIGHLOAD_DOP_INFO = COption::GetOptionString('sns.tools1c' , "CHEXBOX_OFFERS_PROPERTIES_HIGHLOAD_DOP_INFO");
                ...
                foreach($elemProper as $ob) {
                    if($ob['VALUE'] && $ob['CODE'] == $OFFERS_ATTRIBUTES) {
                        ...
                        if(in_array($ob['PROPERTY_TYPE'], array('S','L','N')) && !in_array($ob["DESCRIPTION"],$OFFERS_PROPERTIES_HIGHLOAD_NOT_CREATE)) {
                            ...
                            CSnsToolsEvent::AddPropDirectoryHighLoad(
                                $AddPrpoHighload,
                                $arFields
                            );
                        }
    ...
    function GoodsDirectoryLink(&$arFields=array()) {
        ...
        if($_GET['type'] == 'catalog' && $_GET['mode'] == 'import' && strstr($_GET['filename'], 'import')) {
            ...
            //START
            $GOODS_PROPERTIES_ONE_CAN = COption::GetOptionString('sns.tools1c' , "GOODS_PROPERTIES_ONE_CAN");
            ...
            //START
            $GOODS_PROPERTIES_MUST_NOT = COption::GetOptionString('sns.tools1c' , "GOODS_PROPERTIES_MUST_NOT");
            ...
            $GOODS_PROPERTIES_IBLOCK = COption::GetOptionString('sns.tools1c' , "GOODS_PROPERTIES_IBLOCK");
    ...
    function GoodsDirectoryHighLoadLink(&$arFields=array()) {
        ...
        if($_GET['type'] == 'catalog' && $_GET['mode'] == 'import' && strstr($_GET['filename'], 'import')) {
            ...
            //получим значения которые надо делать
            //START
            $GOODS_PROPERTIES_ONE_CAN = COption::GetOptionString('sns.tools1c' , "GOODS_PROPERTIES_HIGHLOAD_ONE_CAN");
            if(!empty($GOODS_PROPERTIES_ONE_CAN)) {
                $GOODS_PROPERTIES_ONE_CAN = explode(',' , $GOODS_PROPERTIES_ONE_CAN);
                if(!is_array($GOODS_PROPERTIES_ONE_CAN)) {
                    $GOODS_PROPERTIES_ONE_CAN = array($GOODS_PROPERTIES_ONE_CAN);
            ...
            //которые игнорировать
            //START
            $GOODS_PROPERTIES_MUST_NOT = COption::GetOptionString('sns.tools1c' , "GOODS_PROPERTIES_HIGHLOAD_MUST_NOT");
            if(!empty($GOODS_PROPERTIES_MUST_NOT)) {
                $GOODS_PROPERTIES_MUST_NOT = explode(',' , $GOODS_PROPERTIES_MUST_NOT);
                if(!is_array($GOODS_PROPERTIES_MUST_NOT)) {
                    $GOODS_PROPERTIES_MUST_NOT = array($GOODS_PROPERTIES_MUST_NOT);
    ...
    function ProductChangeInfo(&$arFields=array()) {
        ...
        if($_GET['type'] == 'catalog' && $_GET['mode'] == 'import') {
            ...
            $CHEXBOX_ZERO_STOCK = COption::GetOptionString('sns.tools1c' , "CHEXBOX_ZERO_STOCK");
            $CHEXBOX_ZERO_STOCK_DEACTIVE = COption::GetOptionString('sns.tools1c' , "CHEXBOX_ZERO_STOCK_DEACTIVE");
    ...
    function OffersArticleChange(&$arFields=array()) {
        ...
        if($_GET['type'] == 'catalog' && $_GET['mode'] == 'import' && strstr($_GET['filename'], 'offers')) {
            ...
            $arr_xml_id_element = CSnsToolsEvent::GetXmlIdElementByOffers($arFields['IBLOCK_ID']);
            $ID_OFFERS_GOODS = $arr_xml_id_element[$arFields['XML_ID_GOODS']];
            //получим артикулы
            $ArrArticle = CSnsToolsEvent::GetArticleElementByOffers($arFields['IBLOCK_ID']);
    ...
    function OffersMorePhotoChange(&$arFields=array()) {
        ...
        if($_GET['type'] == 'catalog' && $_GET['mode'] == 'import' && strstr($_GET['filename'], 'offers')) {
            $OFFERS_MORE_PHOTO = COption::GetOptionString('sns.tools1c' , "OFFERS_MORE_PHOTO");

            // определим ID товара предложения
            $arr_xml_id_element = CSnsToolsEvent::GetXmlIdElementByOffers($arFields['IBLOCK_ID']);
            $ID_OFFERS_GOODS = $arr_xml_id_element[$arFields['XML_ID_GOODS']];
            //получим изображения
            $arr_more_photo = CSnsToolsEvent::GetMorePhotoElementByOffers($arFields['IBLOCK_ID']);
            ...
            if ($attrib_offers) {
                $OFFERS_MORE_PHOTO_RAZDEL = COption::GetOptionString('sns.tools1c', "OFFERS_MORE_PHOTO_RAZDEL");
                $MORE_ATTRIBUTES = COption::GetOptionString('sns.tools1c', "CHEXBOX_OFFERS_MORE_PHOTO_MORE_ATTRIBUTES");
                $MORE_ATTRIBUTES_COUNT = COption::GetOptionString('sns.tools1c', "CHEXBOX_OFFERS_MORE_PHOTO_MORE_ATTRIBUTES_COUNT");
                $DUPLICATE_PHOTO_OFFERS = COption::GetOptionString('sns.tools1c', "DUPLICATE_PHOTO_OFFERS");
            ...
            //START
            $CHEXBOX_OFFERS_MORE_PHOTO_PREVIEW_PICTURE = COption::GetOptionString('sns.tools1c' , "CHEXBOX_OFFERS_MORE_PHOTO_PREVIEW_PICTURE");
            $CHEXBOX_OFFERS_MORE_PHOTO_DETAIL_PICTURE = COption::GetOptionString('sns.tools1c' , "CHEXBOX_OFFERS_MORE_PHOTO_DETAIL_PICTURE");
    ...
    function MultipleProperiesGoods(&$arFields=array(), $type=false) {
        ...
        if($_GET['type'] == 'catalog' && $_GET['mode'] == 'import' && strstr($_GET['filename'], $type) ) {
            ...
            $resProper = CSnsToolsEvent::GetMultipleProperElement(array(
                'ID' => $arFields['ID'],
                'IBLOCK_ID' => $arFields['IBLOCK_ID'],
                'TYPE' => $type
            ));
    ...
    function DiscontAdd($SETTING=array(), $arFields=array()) {
        ...
        //START
        $siteId = CSnsToolsEvent::GetIblockSiteById($arFields['IBLOCK_ID']);
        foreach ($siteId as $id) {
            if(count($sites) > 0 && !empty($sites)){
                if(!in_array($id,$sites)) continue;
            }
            $discountAll = CSnsToolsEvent::GetAllDiscontTools($id);
            $arrDiscontProduct = CSnsToolsEvent::GetDiscontProduct($id);
            ...
            if($SETTING['DISCONT_VALUE'] && empty($SETTING['DISCONT_ID'])) {
                ...
                $SELECT_DISCONT_LAST_DISCOUNT = COption::GetOptionString('sns.tools1c' , "SELECT_DISCONT_LAST_DISCOUNT");
                if($SELECT_DISCONT_LAST_DISCOUNT){
                    $DiscountAdd['LAST_DISCOUNT'] = $SELECT_DISCONT_LAST_DISCOUNT;
                }


                $SELECT_DISCONT_GROUP_IDS = COption::GetOptionString('sns.tools1c' , "SELECT_DISCONT_GROUP_IDS");
                ...
                $SELECT_DISCONT_CATALOG_GROUP_IDS = COption::GetOptionString('sns.tools1c' , "SELECT_DISCONT_CATALOG_GROUP_IDS");
            ...
            if(!in_array($arFields["ID"], $arrDiscontProduct["DISCONT_INFO"][$SETTING['DISCONT_ID']])) {
                ...
                $allIdElementIblock = CSnsToolsEvent::GetAllIdElementIblock();
            ...
            $CHEXBOX_DISCONT_EMPTY_DELETE = COption::GetOptionString('sns.tools1c' , "CHEXBOX_DISCONT_EMPTY_DELETE");
    ...
    function DiscontGoodsProps(&$arFields=array()) {
        if($_GET['type'] == 'catalog' && $_GET['mode'] == 'import' && strstr($_GET['filename'], 'import')) {
            $DISCONT_CODE = COption::GetOptionString('sns.tools1c' , "DISCONT_CODE");
            $DISCOUNT_CONDITION_BASKET = COption::GetOptionString('sns.tools1c' , "DISCOUNT_CONDITION_BASKET");
            $elemProper = CSnsToolsEvent::GetPropertyElementIblock($arFields['IBLOCK_ID'], $arFields['ID']);
            if(empty($elemProper)){
                $elemProper = array();
            ...
            if(count($elemProper)>0){
                    foreach($elemProper as $ob) {
                        if($ob['CODE'] == $DISCONT_CODE) {
                            if($DISCOUNT_CONDITION_BASKET == 'Y'){
                                CSnsToolsEvent::DiscontsConditionBasket(
                                    array(
    ...
    function DiscontGoodsPrice(&$arFields=array()) {
        ...
        if($_GET['type'] == 'catalog' && $_GET['mode'] == 'import' ) {
            ...
            $DISCONT_PRICE_BASE = COption::GetOptionString('sns.tools1c' , "DISCONT_PRICE_BASE");
            $DISCONT_PRICE_DISCOUNT = COption::GetOptionString('sns.tools1c' , "DISCONT_PRICE_DISCOUNT");
            $DISCOUNT_CONDITION_BASKET = COption::GetOptionString('sns.tools1c' , "DISCOUNT_CONDITION_BASKET");
            ...
            if(!empty($ArrPriceGoods[$DISCONT_PRICE_BASE]['PRICE']) && !empty($ArrPriceGoods[$DISCONT_PRICE_DISCOUNT]['PRICE']) && $ArrPriceGoods[$DISCONT_PRICE_BASE]['PRICE'] > $ArrPriceGoods[$DISCONT_PRICE_DISCOUNT]['PRICE']) {
                ...
                if($ob['VALUE']) {
                    $CSnsToolsEvent_DiscontAdd =  array(
                        'DISCONT_VALUE' => $ob['VALUE'],
                    );
                    if($ArrPriceGoods[$DISCONT_PRICE_BASE]['CURRENCY']){
                        $CSnsToolsEvent_DiscontAdd['DISCONT_CURRENCY'] = $ArrPriceGoods[$DISCONT_PRICE_BASE]['CURRENCY'];
                    }
                    if($DISCOUNT_CONDITION_BASKET == 'Y'){
                        CSnsToolsEvent::DiscontsConditionBasket(
                            $CSnsToolsEvent_DiscontAdd,
                            $arFields
                        );
                    }else{
                        CSnsToolsEvent::DiscontAdd(
    ...
    function OnAfterIBlockElementAdd(&$arFields=array()) {
        ...
        if($CHEXBOX_ARTICLE == 'Y') {
            CSnsToolsEvent::OffersArticleChange($arFields);
        }
        //END
        ...
        if($CHEXBOX_OFFERS_PROPERTIES == 'Y') {
            CSnsToolsEvent::OffersDirectoryLink($arFields);
        }
        //конец

        // свяжем справочники (highload инфоблоки) с торговыми предложениями
        // начало
        $CHEXBOX_OFFERS_PROPERTIES_HIGHLOAD = COption::GetOptionString('sns.tools1c' , "CHEXBOX_OFFERS_PROPERTIES_HIGHLOAD");
        if($CHEXBOX_OFFERS_PROPERTIES_HIGHLOAD == 'Y') {
            CSnsToolsEvent::OffersDirectoryHighLoadLink($arFields);
        }
        //конец
        ...
        if($CHEXBOX_OFFERS_PROPERTIES_STRING == 'Y') {
            CSnsToolsEvent::OffersAttributesSeparate($arFields, 'offers', 'OFFERS_ATTRIBUTES');
        }
        //конец

        // выгрузим свойства характеристик в отдельные свойста типа список
        // начало
        $CHEXBOX_OFFERS_PROPERTIES_LIST = COption::GetOptionString('sns.tools1c' , "CHEXBOX_OFFERS_PROPERTIES_LIST");
        if($CHEXBOX_OFFERS_PROPERTIES_LIST == 'Y') {
            CSnsToolsEvent::OffersAttributesProperList($arFields, 'offers', 'OFFERS_ATTRIBUTES');
        }
        // конец

        // выгрузим свойства реквизитов в отдельные свойста типа строка
        // начало
        $CHEXBOX_TRAITS_STRING = COption::GetOptionString('sns.tools1c' , "CHEXBOX_TRAITS_STRING");
        if($CHEXBOX_TRAITS_STRING == 'Y') {
            CSnsToolsEvent::OffersAttributesSeparate($arFields, 'import', 'GOODS_TRAITS_STRING');
        }
        //конец

        // свяжем свойства товара со справочниками
        //START
        $CHEXBOX_GOODS_PROPERTIES = COption::GetOptionString('sns.tools1c' , "CHEXBOX_GOODS_PROPERTIES");
        if($CHEXBOX_GOODS_PROPERTIES == 'Y') {
            CSnsToolsEvent::GoodsDirectoryLink($arFields);
        }
        //END

        // свяжем свойства товара со справочниками
        //START
        $CHEXBOX_GOODS_PROPERTIES_HIGHLOAD = COption::GetOptionString('sns.tools1c' , "CHEXBOX_GOODS_PROPERTIES_HIGHLOAD");
        if($CHEXBOX_GOODS_PROPERTIES_HIGHLOAD == 'Y') {
            CSnsToolsEvent::GoodsDirectoryHighLoadLink($arFields);
        }
        //END
        ...
        if($CHEXBOX_MULTIPROPER == 'Y') {
            CSnsToolsEvent::MultipleProperiesGoods($arFields, 'import');
        }
        //конец
        ...
        if($CHEXBOX_MULTIPROPER_OFFERS == 'Y') {
            CSnsToolsEvent::MultipleProperiesGoods($arFields, 'offers');
        }
        //конец
        ...
        //START
        $CHEXBOX_OFFERS_MORE_PHOTO = COption::GetOptionString('sns.tools1c' , "CHEXBOX_OFFERS_MORE_PHOTO");
        if($CHEXBOX_OFFERS_MORE_PHOTO == 'Y') {
            CSnsToolsEvent::OffersMorePhotoChange($arFields);
        }
        //END
        ...
        //START
        $CHEXBOX_DISCONT = COption::GetOptionString('sns.tools1c' , "CHEXBOX_DISCONT");
        if($CHEXBOX_DISCONT == 'Y') {
            CSnsToolsEvent::DiscontGoodsProps($arFields);
        }
        //END
        ...
        // START
        $CHEXBOX_DISCONT = COption::GetOptionString('sns.tools1c' , "CHEXBOX_DISCONT_PRICE");
        if($CHEXBOX_DISCONT == 'Y') {
             CSnsToolsEvent::DiscontGoodsPrice($arFields);
        }
        //END
        */
        ...
        $UPDATE_FASET_FILLTER = COption::GetOptionString('sns.tools1c' , "UPDATE_FASET_FILLTER");
    ...
    function OnAfterIBlockElementUpdate(&$arFields=array()){
        ...
        //END
        */
        $ELEMENT_AVAILABLE = COption::GetOptionString('sns.tools1c', "ELEMENT_AVAILABLE");
        if ($ELEMENT_AVAILABLE == 'Y') {
            CSnsToolsEvent::ElementAvailable($arFields);
        }

        $deactivation = COption::GetOptionString('sns.tools1c', "DEACTIVATION");
        if($deactivation == 'Y'){
            CSnsToolsEvent::DeactivationOnProperty($arFields);
        }
    ...
    function OnBeforeIBlockElementAdd(&$arFields=array()) {
        ...
        if($_GET['type'] == 'catalog' && $_GET['mode'] == 'import' && strstr($_GET['filename'], 'import')) {
            ...
            $SELECT_NONE_ADD = unserialize(COption::GetOptionString('sns.tools1c' , "SELECT_NONE_ADD"));
    ...
    function OnBeforeProductAdd(&$arFields=array()) {
        CSnsToolsEvent::ProductChangeInfo($arFields);
    ...
    function OnBeforeIBlockPropertyAdd(&$arFields=array()) {
        ...
        $CHEXBOX_IBLOCK_UNIQUE = COption::GetOptionString('sns.tools1c' , "CHEXBOX_IBLOCK_UNIQUE");
        if($CHEXBOX_IBLOCK_UNIQUE == 'Y') {
            $code = CSnsToolsEvent::GetUniqueProp($arFields['CODE']);
    ...
    function OnSuccessCatalogImport1C() {
        if($_SESSION['BX_CML2_IMPORT']['TOOLS1C']['PRICES']){
            foreach($_SESSION['BX_CML2_IMPORT']['TOOLS1C']['PRICES'] as $key=>$element){
                CSnsToolsEvent::DiscontGoodsPrice($element);
            }
        }
    ...
    function OnIBlockPropertyDelete($ID) {
        CSnsToolsProperty::DeleteByPropertyId($ID);
        CSnsToolsIblock::DeleteByForPropertyId($ID);
        global $CACHE_MANAGER;
        $CACHE_MANAGER->ClearByTag(self::MODULE_ID);
    }
    //END
    ...
    function OnBeforeIBlockPropertyUpdate(&$arFields=array()) {
        ...
        $props1ctools = CSnsToolsProperty::GetByPropertyId($arFields['ID']);
        if(strstr($props1ctools['PROPERTY_TYPE'], ':')) {
            ...
            if($arr_props1ctools[1] == 'directory'){
                ...
                if($props1ctools['PROPERTY_HIGHLOAD_TABLE']!=$arFields["USER_TYPE_SETTINGS"]["TABLE_NAME"]) {
                    ...
                    // START
                    $dbiblo = CSnsToolsIblock::GetList(array(), array('FOR_PROPERTY_ID'=>$arFields['ID']));
                    $resiblo = $dbiblo->NavNext();
                    // END

                    // START
                    CSnsToolsIblock::Update($resiblo['ID'],array('IBLOCK_ID'=>$IblockHighloadNew));
                    CSnsToolsProperty::Update($props1ctools['ID'],array('PROPERTY_HIGHLOAD_TABLE'=>$arFields["USER_TYPE_SETTINGS"]["TABLE_NAME"]));
                    // END
                }
        ...
        if($props1ctools && $arFields['PROPERTY_TYPE']) {
            ...
            if($props1ctools['PROPERTY_TYPE'] != $arFields['PROPERTY_TYPE']){
                global $APPLICATION;
                $APPLICATION->throwException(GetMessage(self::MODULE_ID.'_ERROR_PROPER'), 'SNSIT_TOOLS1_ERROR_PROPER');
    ...
    function OnIBlockDelete($ID=false) {
        CSnsToolsIblock::DeleteByIblockId($ID);
        global $CACHE_MANAGER;
        $CACHE_MANAGER->ClearByTag(self::MODULE_ID);
    }
    //END
    ...
    function OnDiscountDelete($ID=false) {
        CSnsToolsDiscount::DeleteByDiscontId($ID);
        global $CACHE_MANAGER;
        $CACHE_MANAGER->ClearByTag(self::MODULE_ID);
    }
    //END
    ...
    function OnBeforeDiscountUpdate($ID=false, &$arFields=array()) {
        ...
        if($_GET['type'] != 'catalog' && $_GET['mode'] != 'import') {
            ...
            $arrToolsDiscont = CSnsToolsDiscount::GetByDiscontId($ID);
            if($arrToolsDiscont){

                if($arFields['VALUE_TYPE'] != $arrToolsDiscont['TYPE'] || $arFields['VALUE'] != $arrToolsDiscont["DISCONT_VALUE"]){
                    global $APPLICATION;
                    $APPLICATION->throwException(GetMessage(self::MODULE_ID.'_ERROR_DISCONT'), 'SNSIT_TOOLS1_ERROR_DISCONT');
                    return false;
                }
    ...
    function OnBeforeIBlockSectionAdd(&$arFields){
        ...
        if($_GET['type'] == 'catalog' && $_GET['mode'] == 'import' && strstr($_GET['filename'], 'import')) {
            ...
            $SELECT_NONE_ADD_FOR_SECTION = unserialize(COption::GetOptionString('sns.tools1c' , "SELECT_NONE_ADD_FOR_SECTION"));
    ...
    function OnBeforeIBlockSectionUpdate(&$arFields){
        ...
        if($_GET['type'] == 'catalog' && $_GET['mode'] == 'import' && strstr($_GET['filename'], 'import')) {
            ...
            $SELECT_NONE_UPDATE_FOR_SECTION = unserialize(COption::GetOptionString('sns.tools1c' , "SELECT_NONE_UPDATE_FOR_SECTION"));
    ...
    function notDuplicateDocumentsForOrder($event)
    {
        ...
        if ($_GET['type'] == 'sale' && $_GET['mode'] == 'import' && strstr($_GET['filename'], 'Payment')) {
            $NOT_DUPLICATE_PAYMENT = COption::GetOptionString('sns.tools1c', "NOT_DUPLICATE_PAYMENT");
        ...
        if ($_GET['type'] == 'sale' && $_GET['mode'] == 'import' && strstr($_GET['filename'], 'Shipping')) {
            $NOT_DUPLICATE_SHIPMENT = COption::GetOptionString('sns.tools1c' , "NOT_DUPLICATE_SHIPMENT");
    ...
    function KartyLoyalnostiOnBeforeUpdate($event)
    {
        ...
        $addGroup = COption::GetOptionString('sns.tools1c', "ALLOW_CARD_ADD_GROUP_11_2");
        $addUserForGroup = COption::GetOptionString('sns.tools1c', "ALLOW_CARD_ADD_ON_USER_11_2");
        $active = COption::GetOptionString('sns.tools1c', "FIELDS_ACTIVE_CARD_11_2");
        $vladelec = COption::GetOptionString('sns.tools1c', "FIELDS_VLADELETS_CARD_11_2");
        ...
        if ($fields[$active] != 1 && $addGroup == "Y" && $ev == 'N') {
            ...
            $OnUser = COption::GetOptionString('sns.tools1c', "FIELDS_PARTNER_CARD_11_2");
            ...
            $import_code_user = COption::GetOptionString('sns.tools1c', "IMPORT_DISCOUNT_CODE_ON_USER");
            if ($import_code_user == 'Y') {
                $code = COption::GetOptionString('sns.tools1c', "ADD_USER_GROUP_ON_REG_CODE");
                $add_null_card = COption::GetOptionString('sns.tools1c', "ADD_USER_GROUP_ON_REG");
                if($add_null_card == 'Y'){
                    $user_null_card = COption::GetOptionString('sns.tools1c', "ADD_USER_GROUP_ON_REG_DISCOUNT");
    ...
    {
        $active = COption::GetOptionString('sns.tools1c', "ALLOW_DISCOUNT_11_2");
        $active_item = COption::GetOptionString('sns.tools1c', "FIELDS_ACTIVE_DISCOUNT_CARD_11_2");
        $ev = COption::GetOptionString(self::MODULE_ID, "EVENTS");
        $fields = $event->getParameters();
        $fields = $fields['fields'];
        if ($active == 'Y' && $ev == 'N' && $fields[$active_item] != 1) {
            ...
            $roditel_skidki = COption::GetOptionString('sns.tools1c', "FIELDS_GROUP_DISCOUNT_CARD_11_2");
            $roditel_usloviya = COption::GetOptionString('sns.tools1c', "FIELDS_GROUP_CONDITION_DISCOUNT_CARD_11_2");
            $name_skidki = COption::GetOptionString('sns.tools1c', "FIELDS_NAME_DISCOUNT_CARD_11_2");
            $value_skidki = COption::GetOptionString('sns.tools1c', "FIELDS_VALUE_DISCOUNT_CARD_11_2");
            $condition_hl = COption::GetOptionString('sns.tools1c', "CONDITION_DISCOUNT_CARD_11_2");
            $skidki_hl = COption::GetOptionString('sns.tools1c', "DISCOUNT_CARD_11_2");
            $name_condition = COption::GetOptionString('sns.tools1c', "FIELDS_NAME_CONDITION_DISCOUNT_CARD_11_2");
            $cur = COption::GetOptionString('sns.tools1c', "FIELDS_CURRENCY_CONDITION_DISCOUNT_CARD_11_2");
            $value_cond = COption::GetOptionString('sns.tools1c', "FIELDS_VALUE_CONDITION_DISCOUNT_CARD_11_2");
            ...
            if($condition_hl != $skidki_hl) {
               ...
               $check_groups = COption::GetOptionString('sns.tools1c', "NO_GROUPS");
                $discounts = array();
                if($check_groups == 'Y'){
    ...
    {
        if ($_GET['type'] == 'catalog' && $_GET['mode'] == 'import' && strstr($_GET['filename'], $type)) {
            $add = COption::GetOptionString('sns.tools1c', "ATTRIBUTES_FOR_PRODUCT");
            if ($add == 'Y') {
                ...
                if ($product) {
                    ...
                    if (!$ar_props_product = $db_props_product->Fetch()) {
                        $fields = Array(
                            "NAME" => GetMessage("sns.tools1c" . '_CML2_ATTRIBUTES'),
                            "ACTIVE" => "Y",
                            "SORT" => "100",
...
//        if($_GET['type'] == 'reference' && $_GET['mode'] == 'import'){
        ...
        if ($ev == 'N') {
            ...
            global $DB;
            $sql = "SELECT `MESSAGE_ID` FROM `b_module_to_module` WHERE `TO_MODULE_ID`='sns.tools1c'";
            $check = $DB->Query($sql);
            ...
            if (!in_array($hl_update, $arr_event_module) && $hl != '') {
                $eventManager->registerEventHandler("", $hl_update, self::MODULE_ID, "CSnsToolsEvent", "KartyLoyalnostiOnBeforeUpdate");
            ...
            if (!in_array($hl_skidki_update, $arr_event_module) && $hl_skidki != '') {
                $eventManager->registerEventHandler("", $hl_skidki_update, self::MODULE_ID, "CSnsToolsEvent", "addDiscountOn1c");
                EventsTable::add(array('EVENTS'=>$hl_skidki_update, 'FUNCTIONS'=>'addDiscountOn1c'));
            }
            if (!in_array($hl_add, $arr_event_module) && $hl != '') {
                $eventManager->registerEventHandler("", $hl_add, self::MODULE_ID, "CSnsToolsEvent", "KartyLoyalnostiOnBeforeUpdate");
            ...
            if (!in_array($hl_skidki_add, $arr_event_module) && $hl_skidki != '') {
                $eventManager->registerEventHandler("", $hl_skidki_add, self::MODULE_ID, "CSnsToolsEvent", "addDiscountOn1c");
            ...
            foreach ($delete_event_module as $event => $function) {
                $eventManager->unRegisterEventHandler("", $event, 'sns.tools1c', "CSnsToolsEvent", $function);
            }
...
//        echo '<pre>';
    ...
    static function create_profile($array, $payment, $delivery){
        CModule::IncludeModule('sale');
        $create_profile = COption::GetOptionString('sns.tools1c' , "ON_ADD_USER_CREATE_PROFILE_ORDER");
        $db_ptype = CSalePersonType::GetList(Array("SORT" => "ASC"), Array());
        $persone = array();
        while ($ptype = $db_ptype->Fetch())
        {
...
//
    ...
    static function create_order_on_profile($id, $user, $document, $payments, $deliverys, $prof, $inn, $agent){
        ...
        CModule::includeModule('sale');
        $create_order = COption::GetOptionString('sns.tools1c' , "CREATE_ORDER");
...
//        }
    ...
    function checkFileRequest(){
        CModule::includeModule('sale');
        $creat_profile = COption::GetOptionString('sns.tools1c' , "ON_ADD_USER_CREATE_PROFILE");
...
//        global $APPLICATION;
//        $is_update_profile = COption::GetOptionString('sns.tools1c' , "IS_UPDATE_PROFILE");
    ...
    static function update_table_profile($agent = false){
        ...
        while( $table = $table_props->fetch()){
            ...
            if($agent) return 'CSnsToolsEvent::update_table_profile(true);';
    }
    ...
    {
        $unic = COption::GetOptionString('sns.tools1c', "UNIC_INN");

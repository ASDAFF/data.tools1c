...
$sTableID = "tbl_data1ctools_property"; // ID таблицы
  ...
  if(count($arID)>0){  
      foreach($arID as $key=>$ID)
      {
        ...
        case "delete":
          @set_time_limit(0);
          $DB->StartTransaction();
          if(!CDataToolsProperty::Delete($ID))
          {
               
            $DB->Rollback();
            $lAdmin->AddGroupError(GetMessage("rub_del_err"), $ID);
          }  else {
            CDataToolsIblock::DeleteByForPropertyId($arIDfields[$ID]['PROPERTY_ID']);      
          }
          $DB->Commit();
          break;
...
while($arRes = $rsData->NavNext(true, "f_")):
  ...
  if(!$allproperty[$f_PROPERTY_ID]) {
      CDataToolsProperty::DeleteByPropertyId($f_PROPERTY_ID); 
      continue;
  }  
  else{
...
<?
/*  
$re = CDataToolsProperty::GetList(array(), array('PROPERTY_IBLOCK'=>29), array(''));
while($arrRes = $re->Fetch()) { 
    printr($arrRes);    
} 

         
$d = CDataToolsEvent::GetMultipleProperElement(array(
    'ID' => '16365',
    'IBLOCK_ID' => '52'
));
printr($d);   
...
$siteid = CDataToolsEvent::GetIblockSiteById(171);
